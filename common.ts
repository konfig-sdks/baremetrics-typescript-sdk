...
import { readableStreamToString, BaremetricsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BaremetricsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BaremetricsError(e, e.response?.data)
                }
                if (innerError instanceof BaremetricsError) {
                    // Got "BaremetricsError" from the above try block
                    throw innerError;
                }
